...
public:
    ...
    const std::string function(size_t fnwidth) const {
        if (_pretty_function.size() > fnwidth) {
            std::string compact;
            int depth = 0;
            for (char c : _pretty_function) {
                if (c == '>' || c == ')')
                    depth--;
                if (depth == 0)
                    compact.push_back(c);
            ...
            if (compact.size() > fnwidth)
                return compact.substr(0, fnwidth-3) + "...";
            return compact;
        }
        return _pretty_function;
    }
    const std::string &long_function() const {
        return _pretty_function;
    }
...
inline std::ostream &profiler::print(std::ostream &o) const {
    ...
    for (const single_profiler *p : profs) {
        o << std::left << std::setw(fnwidth) << p->function(fnwidth) << " "
            << std::setw(8) << p->line_number() << " " << std::setw(8) << p->calls() << " "
